...
def create_synthetic_data():
    """
    Creates a synthetic dataset of monthly observations over 10 years.
    """
    np.random.seed(123)
    n_years = 10
    start_year = 2010
    ...
    # Define a trend: +2.4 units per year (or +0.2 units per month)
    slope_per_year = 2.4
    slope_per_month = slope_per_year / 12
    base_value = 10
    ...
    for y in range(n_years):
        for m in range(1, 13):
            ...
            # Trend + Random Noise
            val = base_value + (t_months * slope_per_month) + np.random.normal(0, 2)
...
def generate_readme(output_text, plot_filename, p_index, p_frac, slope_index, slope_frac):
    readme_content = f"""# Example 14: Time Vector Nuances (Numeric Data)
...
result_fractional = mk.trend_test(
    x=df['Value'],
    t=t_fractional,
    plot_path='{plot_filename}' # Save plot for this one
)
...
## Interpretation
*   **Identical Significance:** Notice that the p-value is `{p_index:.5f}` in both cases. The choice of time vector units **does not change the statistical conclusion** about whether a trend exists.
*   **Different Slopes:**
    *   Method A gives a slope of **{slope_index:.4f}**. This is the rate of change **per month** (since the time step was 1 month).
    *   Method B gives a slope of **{slope_frac:.4f}**. This is the rate of change **per year**.
...
if __name__ == "__main__":
    ...
    result_index_val = None
    result_fractional_val = None
    ...
    with contextlib.redirect_stdout(f):
        ...
        print(f"Generated {{len(df)}} months of data (10 years).")
        print("True Trend: +2.4 units/year (+0.2 units/month)")
        print()
        ...
        # 3. Run Analysis
        result_index_val = mk.trend_test(x=df['Value'], t=t_index)

        plot_filename = "trend_plot_fractional.png"
        plot_path = os.path.join(os.path.dirname(__file__), plot_filename)

        result_fractional_val = mk.trend_test(x=df['Value'], t=t_fractional, plot_path=plot_path)

        print("--- Comparison of Results ---")
        print(f"Significance (p-value) is IDENTICAL:")
        print(f"  Method A (Index):      {result_index_val.p:.5f}")
        print(f"  Method B (Fractional): {result_fractional_val.p:.5f}")
        print()
        print(f"Sen's Slope MAGNITUDE differs based on 't' units:")
        print(f"  Method A (Index):      {result_index_val.slope:.4f} (approx 0.2 units/step)")
        print(f"  Method B (Fractional): {result_fractional_val.slope:.4f} (approx 2.4 units/year)")
    ...
    readme_content = generate_readme(
        ...
        plot_filename,
        result_index_val.p,
        result_fractional_val.p,
        result_index_val.slope,
        result_fractional_val.slope
    )
    ...
    print(f"Example 14 complete. Artifacts generated in {os.path.dirname(__file__)}")
