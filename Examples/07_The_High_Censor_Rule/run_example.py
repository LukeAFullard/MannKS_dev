...
# --- 3. Move Generated Plots to Correct Directory ---
# The exec() runs in the current process's CWD, so plots are likely in the root
# or wherever the script was called from. We want them in the same directory as this script.
current_dir = os.path.dirname(os.path.abspath(__file__))
plot_files = ['naive_plot.png', 'hicensor_plot.png']

for plot_file in plot_files:
    if os.path.exists(plot_file):
        target_path = os.path.join(current_dir, plot_file)
        shutil.move(plot_file, target_path)
        print(f"Moved {plot_file} to {target_path}")
...
print("Example 7 generated successfully.")
