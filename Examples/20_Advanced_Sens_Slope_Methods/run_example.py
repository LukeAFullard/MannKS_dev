...
with contextlib.redirect_stdout(output_buffer):
    ...
    # Change CWD for execution so plot lands in the right place
...
res_nan = local_scope.get('res_nan')
res_lwp = local_scope.get('res_lwp')
res_ats = local_scope.get('res_ats')
...
### 1. The Slopes
*   **Method 'nan' (Slope={res_nan.slope:.4f})**: Often gives a reasonable estimate but might be slightly unstable if too many pairs are excluded.
*   **Method 'lwp' (Slope={res_lwp.slope:.4f})**: Notice how this is often **lower** (closer to zero) than the others. By forcing ambiguous pairs to zero, it "dampens" the trend signal. If your goal is to match the LWP-TRENDS R script, use this.
*   **Method 'ats' (Slope={res_ats.slope:.4f})**: The Akritas-Theil-Sen estimator often provides the most robust estimate for the "true" trend, using the information from censored ranks more effectively than the simple pairwise heuristics.
