...
def generate_guide():
    """
    Generates a detailed guide for the check_seasonality function.
    """
    # --- 1. Introduction ---
    intro_text = """
    ...
    # --- 3. Execute the code and capture output ---
    f = io.StringIO()
    with redirect_stdout(f):
        exec(code_block)
    output_block = f.getvalue()
    ...
    # --- 6. Assemble the README ---
    readme_content = (
        f"{textwrap.dedent(intro_text).strip()}\n\n"
        ...
        "### Output\n\n"
        "```\n"
        f"{output_block.strip()}\n"
        "```\n\n"
        f"{textwrap.dedent(interpretation_text).strip()}\n\n"
        f"{textwrap.dedent(parameter_guide).strip()}\n"
    )
    ...
    print(f"Successfully generated guide: {readme_path}")
