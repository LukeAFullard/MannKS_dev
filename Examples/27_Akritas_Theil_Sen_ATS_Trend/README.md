...
y_str_obs = np.array([f'<2.5' if c else str(val) for c, val in zip(censored, y_obs)])
...
In this example, the ATS slope is slightly closer to the true underlying slope of 0.08. The generated plot, which always uses the results from the primary method being demonstrated (in this case, ATS), visually confirms the increasing trend.
