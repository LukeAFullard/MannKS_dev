...
def generate_report(df):
    with open(REPORT_FILE, 'w') as f:
        f.write("# Validation 48: Outliers Robustness\n\n")
        f.write("Comparison of Piecewise (OLS) vs. MannKS (Robust) in the presence of **Extreme Outliers** near the breakpoint.\n")
        ...
        plt.plot(t, y, 'ko', alpha=0.6, label='Data (With Outliers)')
        plt.axvline(bp, color='g', linestyle='-', alpha=0.5, label='True BP')
        plt.title("Example Data: Outliers")
        plt.legend()
        img_name = "plot_outliers.png"
        plt.savefig(os.path.join(OUTPUT_DIR, img_name))
        plt.close()

        f.write(f"![Outliers]({img_name})\n\n")
