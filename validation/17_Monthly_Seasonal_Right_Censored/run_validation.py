...
def generate_seasonal_right_censored_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0, censor_threshold=110.0):
    dates = []
    values = []
    for year in range(start_year, start_year + n_years):
        for month in range(1, 13):
            ...
            # Apply right-censoring logic
            # If value is above threshold, censor it
...
def run():
    ...
    mk_kwargs = {
        ...
        'mk_test_method': 'lwp',
        'sens_slope_method': 'lwp',
        'gt_mult': 1.1 # LWP practice for right censored
    }
    ...
    # 1. Strong Increasing Trend (Censored at high end)
    df_inc = generate_seasonal_right_censored_data(n_years=10, trend_slope=2.0, noise_std=1.0, censor_threshold=115.0)
    ...
    scenarios_to_plot.append({'df': df_inc, 'title': 'Strong Increasing (Right-Censored)', 'mk_result': mk_std_inc_plot})

    # 2. Weak Decreasing Trend
    df_dec = generate_seasonal_right_censored_data(n_years=10, trend_slope=-0.5, noise_std=2.0, censor_threshold=105.0)
    ...
    scenarios_to_plot.append({'df': df_dec, 'title': 'Weak Decreasing (Right-Censored)', 'mk_result': mk_std_dec_plot})

    # 3. Stable (No Trend)
    df_stab = generate_seasonal_right_censored_data(n_years=10, trend_slope=0.0, noise_std=1.0, censor_threshold=105.0)
    ...
    scenarios_to_plot.append({'df': df_stab, 'title': 'Stable (Right-Censored)', 'mk_result': mk_std_stab_plot})

    utils.generate_combined_plot(scenarios_to_plot, "V17_Right_Censored_Trend_Analysis.png", "V-17: Monthly Seasonal (Right-Censored) Analysis")
