...
def generate_right_censored_seasonal_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0, censor_limit=105.0, censor_prob=0.3):
    dates = []
    values = []
    for year in range(start_year, start_year + n_years):
        for month in range(1, 13):
            ...
            # If value is greater than censor_limit, censor it at the limit with '>'
...
def run():
    ...
    # 1. Strong Increasing Trend (Right-Censored)
    df_inc = generate_right_censored_seasonal_data(n_years=10, trend_slope=2.0, noise_std=1.0, censor_limit=105.0)
    ...
    # 2. Weak Decreasing Trend (Right-Censored)
    df_dec = generate_right_censored_seasonal_data(n_years=10, trend_slope=-0.5, noise_std=2.0, censor_limit=105.0)
    ...
    # 3. Stable (No Trend) (Right-Censored)
    df_stab = generate_right_censored_seasonal_data(n_years=10, trend_slope=0.0, noise_std=1.0, censor_limit=105.0)
    ...
    utils.generate_combined_plot(scenarios_to_plot, "V17_Right_Censored_Seasonal_Analysis.png", "V-17: Monthly Seasonal (Right-Censored)")
