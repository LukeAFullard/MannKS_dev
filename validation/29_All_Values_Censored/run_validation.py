...
class ValidationUtils:
    ...
    def _prepare_r_dataframe(self, df: pd.DataFrame, is_seasonal: bool = False):
        ...
        # Handle object/string censoring logic
        if df_r_prep['value'].dtype == object:
            # Simple check, assumes prepare_censored_data logic or explicit string like '<5'
            ...
            df_r_prep['RawValue'] = pd.to_numeric(clean_vals, errors='coerce')
        ...
        # Ensure dates are cast correctly in R
    ...
    def run_lwp_r_script(self, df: pd.DataFrame, seasonal: bool = False) -> Dict:
        ...
        try:
            ...
            # Auto-detect frequency for TimeIncr
            # LWP script logic: if multiple unique years > 1, assume annual if we don't have months, but here we enforce standard logic.
            # If standard annual data, use Year.
            is_annual = True
            # Simple check: if month variation exists
            ro.r('month_counts <- length(unique(df_r$Month))')
            m_counts = ro.r('month_counts')[0]
            if m_counts > 1:
                is_annual = False
            ...
            if is_annual and not seasonal:
                 ...
                 ro.r('df_r$TimeIncr <- df_r$Month') # Standard month-based increment
            ...
            # Column names might differ based on function
            slope_col = 'AnnualSenSlope' if 'AnnualSenSlope' in res_df.columns else 'Sen_Slope'

            return {
                'slope': float(res_df[slope_col].iloc[0]),
    ...
    def run_comparison(self, test_id: str, df: pd.DataFrame, scenario_name: str, mk_kwargs: Dict = {}, lwp_mode_kwargs: Dict = {}, true_slope: float = None, seasonal: bool = False) -> Tuple[Dict, object]:
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                ...
                for warning in w:
                    std_warnings.append(str(warning.message))
        except Exception as e:
            print(f"Standard MK failed: {e}")
            from collections import namedtuple
            TrendResult = namedtuple('TrendResult', ['slope', 'p', 'lower_ci', 'upper_ci', 'intercept'])
            mk_std = TrendResult(np.nan, np.nan, np.nan, np.nan, np.nan)
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                ...
                for warning in w:
                    lwp_warnings.append(str(warning.message))
        except Exception as e:
            print(f"LWP Mode failed: {e}")
            from collections import namedtuple
            TrendResult = namedtuple('TrendResult', ['slope', 'p', 'lower_ci', 'upper_ci', 'intercept'])
            mk_lwp = TrendResult(np.nan, np.nan, np.nan, np.nan, np.nan)
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                ...
                for warning in w:
                    ats_warnings.append(str(warning.message))
        except Exception as e:
             from collections import namedtuple
             TrendResult = namedtuple('TrendResult', ['slope', 'p', 'lower_ci', 'upper_ci', 'intercept'])
             mk_ats = TrendResult(np.nan, np.nan, np.nan, np.nan, np.nan)
    ...
    def generate_combined_plot(self, scenarios: List[Dict], filename: str, main_title: str):
        ...
        for ax, scen in zip(axes, scenarios):
            ...
            if 'cen_type' in df.columns:
                ...
                if df['value'].dtype == object and df['value'].astype(str).str.contains('<').any():
                     for val in df['value']:
                         if '<' in str(val): colors.append('red')
                         elif '>' in str(val): colors.append('orange')
                         else: colors.append('black')
                else:
                    colors = ['black'] * len(df)
            ...
            if mk_result is not None and hasattr(mk_result, 'slope') and not np.isnan(mk_result.slope):
                t_numeric = self._get_decimal_year(df)
                intercept = getattr(mk_result, 'intercept', np.nan)
                if not np.isnan(intercept):
                    y_trend = mk_result.slope * t_numeric + intercept
    ...
    def create_report(self, filename='README.md', description=None):
        ...
        with open(report_path, 'w') as f:
            ...
            if self.results:
                ...
                f.write("## Warnings\n")
                for res in self.results:
                    f.write(f"### Test: {res.get('test_id', 'Unknown')}\n")
                    if res.get('mk_py_warnings'):
                        f.write("**Standard Mode Warnings:**\n")
                        for w in res['mk_py_warnings']:
                            f.write(f"- {w}\n")
                    if res.get('lwp_py_warnings'):
                        f.write("**LWP Mode Warnings:**\n")
                        for w in res['lwp_py_warnings']:
                            f.write(f"- {w}\n")
                    if res.get('ats_py_warnings'):
                        f.write("**ATS Mode Warnings:**\n")
                        for w in res['ats_py_warnings']:
                            f.write(f"- {w}\n")
                    f.write("\n")
...
def generate_all_censored_data(n=20, start_year=2000):
    """Generates annual data where every value is censored at <5."""
...
def run():
    ...
    # Scenario 1: All Values Censored
    df_all_censored = generate_all_censored_data(n=20)

    # We expect 'no trend' or handled error.
    res, mk_std = utils.run_comparison(
        test_id="V-29",
        df=df_all_censored,
        scenario_name="all_censored",
        true_slope=0.0
    )
    scenarios.append({
        'df': df_all_censored,
        'title': 'All Values Censored (<5)',
        'mk_result': mk_std
    })

    utils.generate_combined_plot(scenarios, "v29_combined.png", "V-29: All Values Censored Analysis")
    ...
    *   **Comparison:** We aim to verify if MannKS matches the LWP-TRENDS R script's handling of this edge case (whether it returns 0 slope or fails).
    ...
    if warnings_found:
        ...
        conclusion += "No specific warnings were caught, which might be acceptable if the result is robust, but explicit warnings are preferred.\n"
