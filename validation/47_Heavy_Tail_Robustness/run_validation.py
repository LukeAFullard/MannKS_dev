...
def run_comparison(n_iterations=200):
    results = []
    print(f"Running Heavy Tail Robustness Test ({n_iterations} iterations)...")
...
def generate_report(df):
    with open(REPORT_FILE, 'w') as f:
        f.write("# Validation 47: Heavy Tail Robustness\n\n")
        f.write("Comparison of Piecewise (OLS) vs. MannKS (Robust) under **Heavy Tail** noise conditions (t-distribution, df=3).\n")
        ...
        plt.plot(t, y, 'ko', alpha=0.6, label='Data (Heavy Tail)')
        plt.axvline(bp, color='g', linestyle='-', alpha=0.5, label='True BP')
        plt.title("Example Data: Heavy Tail Noise")
        plt.legend()
        img_name = "plot_heavy_tail.png"
        plt.savefig(os.path.join(OUTPUT_DIR, img_name))
        plt.close()

        f.write(f"![Heavy Tail]({img_name})\n\n")
