    ...
    def run_lwp_r_script(self, df: pd.DataFrame, seasonal: bool = False) -> Dict:
        ...
        try:
            ...
            if not seasonal:
                ...
                # LWP script might fail with missing seasons
                cmd = """
                result <- tryCatch({
                    suppressWarnings(
                        SeasonalTrendAnalysis(df_r, do.plot=FALSE, TimeIncrMed=TRUE, UseMidObs=TRUE)
                    )
                }, error = function(e) {
                    print(paste("LWP Error:", e$message))
                    return(NULL)
                })
                """
                ro.r(cmd)
...
def generate_seasonal_missing_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0, missing_months=[7, 8]):
    dates = []
    values = []
...
def run():
    ...
    df_inc = generate_seasonal_missing_data(n_years=10, trend_slope=2.0, noise_std=1.0)
    ...
    scenarios_to_plot.append({'df': df_inc, 'title': 'Strong Increasing (Missing Seasons)', 'mk_result': mk_std_inc_plot})

    # 2. Weak Decreasing Trend
    df_dec = generate_seasonal_missing_data(n_years=10, trend_slope=-0.5, noise_std=2.0)
    ...
    scenarios_to_plot.append({'df': df_dec, 'title': 'Weak Decreasing (Missing Seasons)', 'mk_result': mk_std_dec_plot})

    # 3. Stable (No Trend)
    df_stab = generate_seasonal_missing_data(n_years=10, trend_slope=0.0, noise_std=1.0)
    ...
    scenarios_to_plot.append({'df': df_stab, 'title': 'Stable (Missing Seasons)', 'mk_result': mk_std_stab_plot})

    utils.generate_combined_plot(scenarios_to_plot, "V18_Missing_Seasons_Trend_Analysis.png", "V-18: Seasonal Trend Analysis with Missing Seasons")
