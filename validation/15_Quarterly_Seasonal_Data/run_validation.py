    ...
    def run_lwp_r_script(self, df: pd.DataFrame, seasonal: bool = False) -> Dict:
        ...
        try:
            ...
            if not seasonal:
                ...
                # For quarterly data, ensure Season column is set to Qtr
                if 'Season' not in ro.r('colnames(df_r)'):
                    # Check if we have 4 observations per year implies quarterly
                    ro.r('obs_per_year <- table(df_r$Year)')
                    ro.r('med_obs <- median(obs_per_year)')
                    is_quarterly = ro.r('med_obs >= 3 && med_obs <= 4')[0]

                    if is_quarterly:
                         ro.r('df_r$Season <- df_r$Qtr')
                    else:
        ...
        try:
            ...
            if not seasonal:
                ...
                if 'date' in df.columns:
                    dates = pd.to_datetime(df['date'])
                    # For NADA, we need to manually define seasons if they aren't months
                    # Here we try to detect quarters
                    if len(df) > 0:
                        # Simple quarter logic
                        groups = ((dates.dt.month - 1) // 3 + 1).values
                    else:
...
def generate_quarterly_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0):
    ...
    for year in range(start_year, start_year + n_years):
        for quarter in range(1, 5):
            # Mid-quarter months: Feb, May, Aug, Nov
            month = (quarter - 1) * 3 + 2
...
def run():
    ...
    utils.generate_combined_plot(scenarios_to_plot, "V15_Trend_Analysis.png", "V-15: Quarterly Seasonal Trend Analysis")
