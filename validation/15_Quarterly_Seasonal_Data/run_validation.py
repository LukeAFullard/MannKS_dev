...
def generate_quarterly_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0):
    dates = []
    values = []
    for year in range(start_year, start_year + n_years):
        for quarter in range(1, 5):
            # Mid-quarter dates: Feb 15, May 15, Aug 15, Nov 15
            month = (quarter - 1) * 3 + 2
            date = datetime(year, month, 15)
            t_year = year + (month - 1) / 12.0
            trend_val = trend_slope * (t_year - start_year)
            # Seasonality based on quarter (1-4)
            season_val = season_amp * np.sin(2 * np.pi * (quarter - 1) / 4.0)
...
def run():
    ...
    df_inc = generate_quarterly_data(n_years=10, trend_slope=2.0, noise_std=1.0)
    res_inc, mk_std_inc = utils.run_comparison(
        TEST_ID, df_inc, "strong_increasing",
        seasonal=True,
        mk_kwargs={'season_type': 'quarter', 'period': 4, 'slope_scaling': 'year'},
        lwp_mode_kwargs={'season_type': 'quarter', 'period': 4},
        true_slope=2.0,
        season_col_r='Qtr'
    )
    ...
    df_dec = generate_quarterly_data(n_years=10, trend_slope=-0.5, noise_std=2.0)
    ...
    # 3. Stable (No Trend)
    df_stab = generate_quarterly_data(n_years=10, trend_slope=0.0, noise_std=1.0)
    ...
    utils.generate_combined_plot(scenarios_to_plot, "V15_Quarterly_Analysis.png", "V-15: Quarterly Seasonal Trend Analysis")
