...
### Category 7: Rolling Trend Analysis

**V-36: Rolling Trend Analysis**
*   **Objective:** Comprehensive validation of the rolling trend feature, ensuring internal consistency, correct feature interaction, and robustness.
*   **Sub-cases:**
    *   **V-36a: Basic Correctness:** Compare `rolling_trend_test` results against a manual loop of standard `trend_test` calls on the same data windows. This ensures the windowing and iteration logic is flawless.
    *   **V-36b: Interaction with Aggregation:** Verify that `agg_method` parameters are correctly passed down and applied within each window. Test on high-frequency data aggregated to a larger period.
    *   **V-36c: Interaction with High Censor Rule:** Verify that the `hicensor=True` rule is applied correctly within each window context.
    *   **V-36d: Edge Cases (Small Windows & Insufficient Data):** Test windows that fall below `min_size` or have too few valid points after censoring to ensure graceful handling (warnings/NaNs instead of crashes).
    *   **V-36e: Outlier Robustness:** Introduce a massive outlier in one window and verify that the Sen's slope remains stable compared to a rolling OLS (Ordinary Least Squares) estimate.
    *   **V-36f: Synthetic Reliability:** Validate against a known synthetic signal (e.g., a sine wave trend) to ensure the rolling slope accurately tracks the changing derivative of the signal.
