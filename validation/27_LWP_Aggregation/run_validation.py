...
def generate_aggregated_data(n_years=5, start_year=2000, trend_slope=0.0, noise_std=1.0):
    """Generates monthly data with 3 observations per month."""
    dates = []
    values = []
    # 3 observations per month: 5th, 15th, 25th
    days = [5, 15, 25]
    ...
    for year in range(start_year, start_year + n_years):
        for month in range(1, 13):
            for d in days:
                ...
                # Trend calculation based on fractional year
                t_year = year + (month - 1) / 12.0 + (d - 1) / 365.25
                trend_val = trend_slope * (t_year - start_year)
                noise = np.random.normal(0, noise_std)
                # Base value 10
                val = 10 + trend_val + noise
                dates.append(date)
                values.append(val)
...
def run():
    ...
    # 1. Strong Increasing
    df_inc = generate_aggregated_data(n_years=5, trend_slope=2.0, noise_std=0.5)
    ...
    scenarios_to_plot.append({'df': df_inc, 'title': 'Strong Increasing (Multiple Obs/Month)', 'mk_result': mk_std_inc_plot})

    # 2. Weak Decreasing
    df_dec = generate_aggregated_data(n_years=5, trend_slope=-0.5, noise_std=1.0)
    ...
    scenarios_to_plot.append({'df': df_dec, 'title': 'Weak Decreasing (Multiple Obs/Month)', 'mk_result': mk_std_dec_plot})

    # 3. Stable
    df_stab = generate_aggregated_data(n_years=5, trend_slope=0.0, noise_std=1.0)
    ...
    scenarios_to_plot.append({'df': df_stab, 'title': 'Stable (Multiple Obs/Month)', 'mk_result': mk_std_stab_plot})

    utils.generate_combined_plot(scenarios_to_plot, "V27_LWP_Aggregation.png", "V-27: LWP Aggregation Analysis")
