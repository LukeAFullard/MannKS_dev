...
class ValidationUtils:
    ...
    def run_lwp_r_script(self, df: pd.DataFrame, seasonal: bool = False, season_col: str = 'Month') -> Dict:
        ...
        Args:
            ...
            seasonal: Whether to run a seasonal trend test.
            season_col: The name of the column to use as the season (default: 'Month').
                        For quarterly data, this should be set to 'Qtr' (if GetMoreDateInfo produces it)
                        or 'Month' if we are tricking it.
        """
        ...
        try:
            ...
            # This function adds Month, Qtr, etc.
            ...
            if is_annual and not seasonal:
                 ...
                 # Default logic used Month, but for V15 (Quarterly) we might need Qtr
                 if season_col == 'Qtr':
            ...
            if not seasonal:
                ...
                # Set the Season column for SeasonalTrendAnalysis
    ...
    def run_comparison(self, test_id: str, df: pd.DataFrame, scenario_name: str, mk_kwargs: Dict = {}, lwp_mode_kwargs: Dict = {}, true_slope: float = None, seasonal: bool = False, season_col_r: str = 'Month') -> Tuple[Dict, object]:
        ...
        # Pass season_col_r to run_lwp_r_script
...
def generate_censored_seasonal_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0, censor_limit=100.0, censor_prob=0.3):
    dates = []
    values = []
    for year in range(start_year, start_year + n_years):
        for month in range(1, 13):
            ...
            # Apply censoring
            if raw_val < censor_limit and np.random.rand() < censor_prob:
                val = f"<{censor_limit}"
            else:
                val = raw_val
...
def fix_mk_result_for_plotting(mk_res, df):
    ...
    # Handle censored values for plotting (simple substitution)
...
def run():
    ...
    # 1. Strong Increasing Trend (Censored)
    # Start lower so we get censoring
    df_inc = generate_censored_seasonal_data(n_years=10, trend_slope=2.0, noise_std=1.0, censor_limit=105.0)
    ...
    # 2. Weak Decreasing Trend (Censored)
    df_dec = generate_censored_seasonal_data(n_years=10, trend_slope=-0.5, noise_std=2.0, censor_limit=105.0)
    ...
    # 3. Stable (No Trend) (Censored)
    df_stab = generate_censored_seasonal_data(n_years=10, trend_slope=0.0, noise_std=1.0, censor_limit=105.0)
    ...
    utils.generate_combined_plot(scenarios_to_plot, "V16_Censored_Seasonal_Analysis.png", "V-16: Monthly Seasonal (Left-Censored)")
