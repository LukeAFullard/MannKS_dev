...
class ValidationUtils:
    ...
    def run_comparison(self, test_id: str, df: pd.DataFrame, scenario_name: str, mk_kwargs: Dict = {}, lwp_mode_kwargs: Dict = {}, true_slope: float = None, seasonal: bool = False) -> Tuple[Dict, object]:
        ...
        if seasonal:
             # Remove conflicting arguments for ATS
             ats_kwargs = mk_kwargs.copy()
             if 'sens_slope_method' in ats_kwargs:
                 del ats_kwargs['sens_slope_method']
             mk_ats = mk.seasonal_trend_test(x_std, t_ats, sens_slope_method='ats', **ats_kwargs)
        else:
...
def generate_seasonal_censored_data(n_years=10, start_year=2000, trend_slope=0.0, noise_std=1.0, season_amp=5.0, censor_threshold=100.0, censor_prob=0.3):
    dates = []
    values = []
    for year in range(start_year, start_year + n_years):
        for month in range(1, 13):
            ...
            # If value is below threshold, censor it
            # We add some randomness to censoring to simulate varying detection limits if needed,
            # but for this test, a fixed threshold is clearer.
            # To ensure we have enough censored data to test, we can force censoring on low values
            # or just blindly censor a percentage. Let's censor based on value.

            if raw_val < censor_threshold:
                 val = f"<{censor_threshold}"
            else:
                 val = raw_val
...
def fix_mk_result_for_plotting(mk_res, df):
    # This function is trickier with censored data as we can't easily plot the "raw" censored string.
    # We'll plot the numeric part.
    dates = pd.to_datetime(df['date'])
    t_years = (dates - pd.Timestamp("1970-01-01")).dt.days / 365.25
    t_years = t_years.values

    y_raw = df['value'].astype(str).str.replace('<', '').str.replace('>', '').astype(float).values
...
def run():
    ...
    # Common kwargs for censored analysis
    mk_kwargs = {
        'season_type': 'month',
        'period': 12,
        'slope_scaling': 'year',
        'mk_test_method': 'lwp', # Using LWP method by default for robustness comparison
        'sens_slope_method': 'lwp',
        'lt_mult': 0.5 # Standard LWP practice
    }
    ...
    # 1. Strong Increasing Trend (Censored at low end)
    # Threshold at 100 ensures the start of the series (around 100) has censoring
    df_inc = generate_seasonal_censored_data(n_years=10, trend_slope=2.0, noise_std=1.0, censor_threshold=105.0)
    res_inc, mk_std_inc = utils.run_comparison(
        TEST_ID, df_inc, "strong_increasing",
        seasonal=True,
        mk_kwargs=mk_kwargs,
        lwp_mode_kwargs=lwp_mode_kwargs,
        true_slope=2.0
    )
    mk_std_inc_plot = fix_mk_result_for_plotting(mk_std_inc, df_inc)
    scenarios_to_plot.append({'df': df_inc, 'title': 'Strong Increasing (Left-Censored)', 'mk_result': mk_std_inc_plot})

    # 2. Weak Decreasing Trend (Censored at low end)
    # Decreasing trend means values drop INTO censoring range
    df_dec = generate_seasonal_censored_data(n_years=10, trend_slope=-0.5, noise_std=2.0, censor_threshold=95.0)
    ...
    scenarios_to_plot.append({'df': df_dec, 'title': 'Weak Decreasing (Left-Censored)', 'mk_result': mk_std_dec_plot})

    # 3. Stable (No Trend)
    df_stab = generate_seasonal_censored_data(n_years=10, trend_slope=0.0, noise_std=1.0, censor_threshold=100.0)
    ...
    scenarios_to_plot.append({'df': df_stab, 'title': 'Stable (Left-Censored)', 'mk_result': mk_std_stab_plot})

    utils.generate_combined_plot(scenarios_to_plot, "V16_Censored_Trend_Analysis.png", "V-16: Monthly Seasonal (Left-Censored) Analysis")
