...
def generate_long_run_data(n=30, start_year=2000, seed=42):
    """Generates data with a long run of identical values (70% flat)."""
    np.random.seed(seed)
    dates = [datetime(start_year + i, 1, 1) for i in range(n)]
    t = np.arange(n)

    # 70% of data is identical (e.g., 5.0)
    num_identical = int(0.7 * n)
    values = np.zeros(n)

    # First part random, middle part flat, last part random
    start_idx = 5
    end_idx = start_idx + num_identical

    values[:start_idx] = np.random.normal(5, 1, start_idx)
    values[start_idx:end_idx] = 5.0 # The long run
    values[end_idx:] = np.random.normal(5, 1, n - end_idx)
...
def run():
    ...
    # Scenario 1: Long Run of Identical Values
    df_long_run = generate_long_run_data(n=30, seed=42)
    ...
    scenarios.append({
        'df': df_long_run,
        'title': 'Long Run of Identical Values (70%)',
        'mk_result': mk_std
    })

    utils.generate_combined_plot(scenarios, "v30_combined.png", "V-30: Long Run of Identical Values Analysis")
    ...
    *   **Comparison:** We aim to verify if MannKS detects this pattern similarly to the LWP-TRENDS R script.
    ...
    if any("Long run of single value" in w for w in warnings_found):
        ...
        conclusion += "However, it **did not issue** a specific warning for the long run of identical values in the main output stream captured here. Users should check the 'analysis_notes' field in the result object for detailed data quality flags.\n"
