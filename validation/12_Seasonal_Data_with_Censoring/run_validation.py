...
The plot generated by `mannkensen` is shown below. The censored data points are highlighted in a different color (typically red), illustrating their position at the lower end of the seasonal cycles.
...
The **MannKenSen (LWP Mode)**, configured with `agg_method='lwp'`, `ci_method='lwp'`, and `sens_slope_method='lwp'`, produces results that are very closely aligned with the **LWP-TRENDS R Script**. This demonstrates that the LWP-emulation settings in `mannkensen` are effective for complex scenarios involving both seasonality and censoring.

The **MannKenSen (Standard)** mode, which uses more robust statistical defaults for handling censored data, produces a slightly different but directionally consistent result, as expected. This validation confirms that the seasonal test function integrates correctly with the censored data preprocessing and analysis logic.
"""
