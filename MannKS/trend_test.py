        ...
        if surrogate_method != 'none':
            ...
            if 'original_index' in data_filtered.columns:
                ...
                if agg_method != 'none':
                    ...
                    # We only check for array-like arguments that likely correspond to original data.
                    for k, v in kwargs_base.items():
                        if hasattr(v, '__len__') and len(v) == n_orig and not isinstance(v, str):
                            raise ValueError(
                                f"Surrogate argument '{k}' (length {len(v)}) cannot be automatically mapped when aggregation "
